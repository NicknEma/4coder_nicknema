...
internal void F4_CPP_ParseMacroDefinition(F4_Index_ParseCtx *ctx) {
    Token *name = 0;
...
internal b32 F4_CPP_SkipParseBody(F4_Index_ParseCtx *ctx) {
...
procedure void F4_CPP_ParseStructOrUnionBody(F4_Index_ParseCtx *ctx, F4_Index_NoteFlags note_flags) {
    Token *name = 0;
    b32 valid = false;
...
function b32 F4_CPP_ParseFunctionBody(F4_Index_ParseCtx *ctx, b32 *prototype_ptr) {
    b32 valid = false;
    b32 prototype = false;
...
procedure void F4_CPP_ParseEnumBody(F4_Index_ParseCtx *ctx) {
...
internal FILE_INDEXER(F4_CPP_IndexFile) {
    ...
    for (b32 handled = false; !ctx->done; ) {
        handled = false;
        ...
        //~ NOTE(rjf): Structs
            ...
            handled = true;
            F4_CPP_ParseStructOrUnionBody(ctx, F4_Index_NoteFlag_ProductType);
        } else if (F4_Index_ParsePattern(ctx, "%t%t", "typedef", "struct")) {
            handled = true;
            F4_CPP_ParseStructOrUnionBody(ctx, 0);
            if (F4_Index_ParsePattern(ctx, "%k", TokenBaseKind_Identifier, &name)) {
        ...
        //~ NOTE(rjf): Unions
            ...
            handled = true;
            F4_CPP_ParseStructOrUnionBody(ctx, F4_Index_NoteFlag_SumType);
										...
										"(")))
        {
            handled = true;
            b32 prototype = false;
            if (F4_CPP_ParseFunctionBody(ctx, &prototype)) {
