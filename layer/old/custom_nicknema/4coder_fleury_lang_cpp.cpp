#ifndef FCODER_FLEURY_LANG_CPP_CPP
#define FCODER_FLEURY_LANG_CPP_CPP

procedure void F4_CPP_ParseMacroDefinition(F4_Index_ParseCtx *ctx) {
    Token *name = 0;
...
procedure b32 F4_CPP_SkipParseBody(F4_Index_ParseCtx *ctx) {
    b32 body_found = 0;
    int nest = 0;
...
procedure b32 F4_CPP_ParseDecl(F4_Index_ParseCtx *ctx, Token **name) {
    Token *base_type = 0;
    b32 result = (F4_Index_ParsePattern(ctx, "%k%o%k%o%t",
...
procedure void F4_CPP_ParseStructOrUnionBodyIFuckingHateCPlusPlus(F4_Index_ParseCtx *ctx, F4_Index_NoteFlags note_flags) {
    Token *name = 0;
    b32 valid = 0;
    b32 need_end_name = 0;
...
procedure b32 F4_CPP_ParseFunctionBodyIFuckingHateCPlusPlus(F4_Index_ParseCtx *ctx, b32 *prototype_ptr) {
    b32 valid = 0;
    b32 prototype = 0;
...
procedure void F4_CPP_ParseEnumBodyIFuckingHateCPlusPlus(F4_Index_ParseCtx *ctx) {
...
procedure LANGUAGE__INDEX_FILE(F4_CPP_IndexFile) {
    int scope_nest = 0;
        ...
        //~ NOTE(rjf): Pure Typedefs
            ...
            for (;!ctx->done;) {
                if (F4_Index_ParsePattern(ctx, "%t", "(")) {
                    ...
                    if (namenote != 0 && namenote->kind == F4_Index_NoteKind_Type && namenote->flags & F4_Index_NoteFlag_SumType)
                        sum_type = 1;
                } else if (F4_Index_ParsePattern(ctx, "%t", ";")) {
                    break;
                } else {
...
internal LANGUAGE__GET_POSITIONAL_CONTEXT(F4_CPP_GetPositionalContext) {
    int count = 0; cast(void)count;
    Language__Positional_Context_Data *first = 0;
    Language__Positional_Context_Data *last = 0; cast(void)last;
...
internal LANGUAGE__HIGHLIGHT(F4_CPP_Highlight) {
}

#endif // FCODER_FLEURY_LANG_CPP_CPP
