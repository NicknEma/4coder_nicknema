#ifndef FCODER_RFLEURY_LANG_JAI_CPP
#define FCODER_RFLEURY_LANG_JAI_CPP
...
internal LANGUAGE__INDEX_FILE(F4_Jai_IndexFile) {
    int scope_nest = 0;
    for (;!ctx->done;) {
        Token *name = 0;
        ...
        if (F4_Index_RequireTokenKind(ctx, TokenBaseKind_Identifier, &name, flags)) {
            ...
            if (F4_Index_RequireToken(ctx, str8_lit("::"), flags)) {
                // NOTE(rjf): Procedures
                if (F4_Index_PeekToken(ctx, str8_lit("(")) ||
                    (F4_Index_RequireToken(ctx, str8_lit("inline"), flags) &&
...
internal Token *_F4_Jai_FindDecl(Application_Links *app, Buffer_ID buffer, i64 pos, Token *decl_name) {
    ...
    for (;;) {
        ...
        if (token) {
            if (scope_nest == 0 && token->sub_kind == TokenJaiKind_Colon && token_it_dec_non_whitespace(&it)) {
...
internal LANGUAGE__GET_POSITIONAL_CONTEXT(F4_Jai_GetPositionalContext) {
    int count = 0;
    Language__Positional_Context_Data *first = 0;
    Language__Positional_Context_Data *last = 0;
...
internal LANGUAGE__HIGHLIGHT(F4_Jai_Highlight) {
}

#endif // FCODER_RFLEURY_LANG_JAI_CPP
