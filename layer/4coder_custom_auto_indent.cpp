...
internal b32 indent__unfinished_statement(Token *token, Nest *current_nest, b32 require_statement_closer) {
    b32 result = false;
...
internal i64 *get_indentation_array(Application_Links *app, Arena *arena, Buffer_ID buffer, Range_i64 lines, nne::Indent_Flag flags, i32 tab_width, i32 indent_width) {
...
internal b32 auto_indent_buffer(Application_Links *app, Buffer_ID buffer, Range_i64 pos, nne::Indent_Flag flags, i32 tab_width, i32 indent_width){
...
internal b32 auto_indent_buffer(Application_Links *app, Buffer_ID buffer, Range_i64 pos, nne::Indent_Flag flags) {
...
internal b32 auto_indent_buffer(Application_Links *app, Buffer_ID buffer, Range_i64 pos) {
...
internal void auto_indent_whole_file(Application_Links *app) {
...
internal void auto_indent_line_at_cursor(Application_Links *app) {
	View_ID view = get_active_view(app, Access_ReadWriteVisible);
    Buffer_ID buffer = view_get_buffer(app, view, Access_ReadWriteVisible);
...
internal void auto_indent_range(Application_Links *app) {
	View_ID view = get_active_view(app, Access_ReadWriteVisible);
    Buffer_ID buffer = view_get_buffer(app, view, Access_ReadWriteVisible);
...
internal void write_text_and_auto_indent(Application_Links *app) {
    ...
    if (insert.str != 0 && insert.size > 0){
        ...
        if (do_auto_indent){
            ...
            nne::auto_indent_buffer(app, buffer, pos, 0);
            move_past_lead_whitespace(app, view, buffer);
        }
        else{
            write_text_input(app);
