...
namespace nne {
	...
	function void render_buffer(Application_Links *app, View_ID view_id, Face_ID face_id, Buffer_ID buffer, Text_Layout_ID text_layout_id, Rect_f32 rect, Frame_Info frame_info) {
		...
		{
			Buffer_ID compilation_buffer = get_buffer_by_name(app, string_u8_litexpr("*compilation*"), Access_Always);
			render_error_annotations(app, buffer, text_layout_id, compilation_buffer);
			// @Cleanup: Extract the config check and put it here.
		}
		...
		
		{
			...
			if (should_highlight_token_occurrences) {
					...
					for (;;) {
						...
						if (token->kind == TokenBaseKind_Identifier) {
							...
							if (range_contains(token_range, view_get_cursor_pos(app, view_id))) {
								...
								render_range_highlight(app, view_id, text_layout_id, token_range, Range_Highlight_Kind::Underline, fcolor_resolve(fcolor_id(fleury_color_token_highlight)));
								...
								render_range_highlight(app, view_id, text_layout_id, token_range, Range_Highlight_Kind::Minor_Underline, fcolor_resolve(fcolor_id(fleury_color_token_minor_highlight)));
				...
				if (token && token->kind == TokenBaseKind_Identifier) {
					render_range_highlight(app, view_id, text_layout_id, Ii64(token->pos, token->pos + token->size), Range_Highlight_Kind::Underline, fcolor_resolve(fcolor_id(fleury_color_token_highlight)));
				}
			}
		}
		...
		{
			//- Render flashes.
			render_flashes(app, view_id, text_layout_id);
		}
		...
		
		{
			...
			if (is_active_view && fcoder_mode == FCoderMode_Original) {
				highlight_cursor_mark_range(app, view_id);
			}
		}
		...
		
		{
			//- Draw cursor.
			switch (fcoder_mode) {
				case FCoderMode_Original:    { render_cursor_emacs_style(app, view_id, is_active_view, buffer, text_layout_id, cursor_roundness, mark_thickness, frame_info);   } break;
				case FCoderMode_NotepadLike: { render_cursor_notepad_style(app, view_id, is_active_view, buffer, text_layout_id, cursor_roundness, mark_thickness, frame_info); } break;
			}
		}
		...
		
		{
			...
			render_close_brace_annotation(app, buffer, text_layout_id, cursor_pos);
		}
		
		{
			//- Draw vertical brace lines, as an additional scope helper.
			render_brace_vertical_lines(app, buffer, view_id, text_layout_id, cursor_pos);
		}
		...
		if (active_view == view_id) {
			...
			{
				render_positional_context_tooltips(app, view_id, buffer, text_layout_id, cursor_pos);
			}
			
			{
				...
				for (int i = 0; i < global_tooltip_count; i += 1) {
					...
					draw_tooltip_rect(app, tooltip_rect);
		...
		
		{
			...
			if (!view_get_is_passive(app, view_id) && !is_active_view) {
				render_code_peek(app, view_id, face_id, buffer, frame_info);
			}
		}
	...
	function void render(Application_Links *app, Frame_Info frame_info, View_ID view_id) {
		refresh_recent_files_view(app, view_id);
		
		ProfileScope(app, "Render");
		Scratch_Block scratch(app);
		...
		//~ Draw margin.
		{
			...
			if (def_get_config_b32(vars_save_string_lit("f4_margin_use_mode_color")) && is_active_view) {
				color = get_color(app, make_cursor_color_context(power_mode.enabled ? Color_Flag_PowerMode : 0, global_keybinding_mode));
			}
			draw_margin(app, view_rect, region, color);
		}
	...
	function void do_full_lex_async_internal(Async_Context *actx, Buffer_ID buffer_id) {
		...
		i32 limit_factor = 10000; // @Cleanup: Magic numbers!
		...
		if (language == 0) {
			// Fall back to C++ if we don't have a proper language.
	...
	function void do_full_lex_async(Async_Context *actx, String_Const_u8 data) {
		if (data.size == sizeof(Buffer_ID)) {
			Buffer_ID buffer = *cast(Buffer_ID *)data.str;
	...
	function i32 begin_buffer(Application_Links *app, Buffer_ID buffer_id) {
		...
		//- Treat as code if the config tells us to.
		// @Cleanup: What else could it be? We just set it to false...   Maybe just for consistency with the later sections?
		if (treat_as_code == false) {
			if (file_name.size > 0) {
				String_Const_u8 treat_as_code_string = def_get_config_string(scratch, vars_save_string_lit("treat_as_code"));
				String_Const_u8_Array extensions = parse_extension_line_to_extension_list(app, scratch, treat_as_code_string);
				String_Const_u8 ext = string_file_extension(file_name);
				for (i32 i = 0; i < extensions.count; i += 1) {
		...
		//- Treat as code for *calc* and *peek* buffers.
		if (treat_as_code == false) {
		...
		//- Treat as code if we've identified the language of a file.
		if (treat_as_code == false) {
			F4_Language *language = language_from_buffer(app, buffer_id);
			if (language) {
				treat_as_code = true;
	...
	function Layout_Item_List layout_internal(Application_Links *app, Arena *arena, Buffer_ID buffer, Range_i64 range, Face_ID face, f32 width, Layout_Virtual_Indent virt_indent) {
		...
		f32 tab_width = cast(f32)def_get_config_u64(app, vars_save_string_lit("default_tab_width"));
		...
		if (text.size == 0) {
			...
			for (; ptr < end_ptr; ) {
				Character_Consume_Result consume = utf8_consume(ptr, cast(u64)(end_ptr - ptr));
	...
	function i32 buffer_edit_range(Application_Links *app, Buffer_ID buffer_id, Range_i64 new_range, Range_Cursor old_cursor_range) {
		ProfileScope(app, "Buffer Edit Range");
		...
		if (ptr && ptr->tokens) {
			...
			if (token_index_resync_guess - token_index_first >= 4000) { // @Cleanup: Magic numbers!
				do_full_relex = true;
			} else {
