...
internal String8 read_syntax_options_string(void) {
...
internal b32 argb_is_valid(ARGB_Color color) {
...
#define Fill_From_Flag(flag) do{ t = syntax_flag_transitions[get_single_bit_offset(flag)]; }while(0)
    ...
    //~ Cursor Color
        ...
        if (ctx.flags & Color_Flag_Macro) {
            color = argb_from_id(table, fleury_color_cursor_macro);
        } else if (ctx.flags & Color_Flag_PowerMode) {
