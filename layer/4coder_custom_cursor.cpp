#ifndef FCODER_CUSTOM_CURSOR_CPP
#define FCODER_CUSTOM_CURSOR_CPP
...
namespace nne {
	...
	// @Rename(ema): What?
	function void render_cursor_shape(Application_Links *app, Rect_f32 rect, f32 roundness, f32 thickness, ARGB_Color color, Cursor_Type type) {
	...
	function void do_cursor_interpolation(Application_Links *app, Frame_Info frame_info, Rect_f32 *rect, Rect_f32 *last_rect, Rect_f32 target) {
		*last_rect = *rect;
	...
	function void render_cursor_emacs_style(Application_Links *app, View_ID view_id, b32 is_active_view, Buffer_ID buffer, Text_Layout_ID text_layout_id, f32 roundness, f32 outline_thickness, Frame_Info frame_info) {
		...
		if (!argb_is_valid(inactive_cursor_color)) {
			...
			for (int i = 0; i < 1/*global_cursor_count*/; ++i) {
				...
				// Draw cursor.
				{
					if (is_active_view) {
						...
						do_cursor_interpolation(app, frame_info, &global_cursor_rect, &global_last_cursor_rect, target_cursor);
						...
						do_cursor_interpolation(app, frame_info, &global_mark_rect, &global_last_mark_rect, target_mark);
					}
					
					// Draw main cursor.
					{
						render_cursor_shape(app, global_cursor_rect, roundness, 4.f, cursor_color, cursor_type);
						render_cursor_shape(app, target_cursor, roundness, 4.f, cursor_color, cursor_type);
					}
					...
					for (int glow = 0; glow < 20; ++glow) {
						...
						if (alpha > 0) {
							...
							glow_rect.x1 += glow;
							glow_rect.y1 += glow;
							render_cursor_shape(app, glow_rect, roundness + glow*0.7f, 2.f, fcolor_resolve(fcolor_change_alpha(fcolor_argb(cursor_color), alpha)), cursor_type);
						} else { break; }
					}
				...
				render_cursor_shape(app, global_mark_rect, roundness, 2.f, fcolor_resolve(fcolor_change_alpha(fcolor_argb(mark_color), 0.5f)), mark_type);
				render_cursor_shape(app, target_mark, roundness, 2.f, fcolor_resolve(fcolor_change_alpha(fcolor_argb(mark_color), 0.75f)), mark_type);
			}
		}
	...
	// @Unused...
	function b32 F4_Cursor_DrawHighlightRange(Application_Links *app, View_ID view_id, Buffer_ID buffer, Text_Layout_ID text_layout_id, f32 roundness) {
	...
	function void render_cursor_notepad_style(Application_Links *app, View_ID view_id, b32 is_active_view, Buffer_ID buffer, Text_Layout_ID text_layout_id, f32 roundness, f32 outline_thickness, Frame_Info frame_info) {
		...
		if (!has_highlight_range) {
			...
			// Draw cursor.
			{
				...
				if (is_active_view) {
					do_cursor_interpolation(app, frame_info, &global_cursor_rect, &global_last_cursor_rect, rect);
				}
...
#if 0
	function void F4_RenderMarkAnnotation(Application_Links *app, Buffer_ID buffer, Text_Layout_ID text_layout_id, View_ID view_id, b32 is_active_view) {
...
#endif // FCODER_CUSTOM_CURSOR_CPP
