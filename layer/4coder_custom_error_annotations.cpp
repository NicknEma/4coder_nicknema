#ifndef FCODER_CUSTOM_ERROR_ANNOTATIONS_CPP
#define FCODER_CUSTOM_ERROR_ANNOTATIONS_CPP
...
namespace nne {
	...
	function void render_error_annotations(Application_Links *app, Buffer_ID buffer, Text_Layout_ID text_layout_id, Buffer_ID jump_buffer) {
		...
		ProfileScope(app, "Error Annotations");
		
		Heap *heap = &global_heap;
		Scratch_Block scratch(app);
		
		Locked_Jump_State jump_state = {}; {
			ProfileScope(app, "Error Annotations (Get Locked Jump State)");
		...
		if (jump_buffer != 0 && jump_state.view != 0) {
			...
			{
				ProfileScope(app, "Error Annotations (Buffer Get Managed Scope)");
			...
			{
				ProfileScope(app, "Error Annotations (Get Managed Scope)");
			...
			{
				ProfileScope(app, "Error Annotations (Scope Attachment)");
				buffer_markers_object = scope_attachment(app, comp_scope, sticky_jump_marker_handle, Managed_Object);
			}
			
			// Get buffer markers (locations where jumps point at).
			i32 buffer_marker_count = 0;
			Marker *buffer_markers = 0;
			{
				ProfileScope(app, "Error Annotations (Load Managed Object Data)");
			...
			for (i32 i = 0; i < buffer_marker_count; i += 1) {
				ProfileScope(app, "Error Annotations (Buffer Loop)");
				...
				if (code_line_number != last_line) {
					...
					// Render annotation.
					{
						...
						Rect_f32 last_character_on_line_rect = text_layout_character_on_screen(app, text_layout_id, get_line_end_pos(app, buffer, code_line_number) - 1);
						...
						if (range_size(y) > 0.f) {
							...
							if(mouse_state.x >= region.x0 && mouse_state.x <= region.x1 && mouse_state.y >= y.min && mouse_state.y <= y.max) {
								push_tooltip(jump_line, 0xffff0000);
							}
						}
					}
				}
...
#endif // FCODER_CUSTOM_ERROR_ANNOTATIONS_CPP
