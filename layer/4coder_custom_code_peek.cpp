...
namespace nne {
	...
	global b32 global_code_peek_open = false;
	
	function void render_code_peek(Application_Links *app, View_ID view_id, Face_ID face_id, Buffer_ID buffer, Frame_Info frame_info) {
...
peek_count += 1;\
		...
		// Push identifiers left around in *peek*.
		{
...
#undef PushPeek
		...
		f32 peek_height = cast(f32)((view_rect.y1 - view_rect.y0) * (0.5f + 0.4f*(clamp_top(peek_count / 4, 1)))) / peek_count;
		Rect_f32 rect = {};
		{
...
CUSTOM_COMMAND_SIG(clear_code_peek_buffer)
