#ifndef FCODER_FLEURY_INDEX_CPP
#define FCODER_FLEURY_INDEX_CPP

global F4_Index_State f4_index = {};

procedure void F4_Index_Initialize(void) {
    f4_index.mutex = system_mutex_make();
    f4_index.arena = make_arena_system(KB(16));
}

procedure void F4_Index_Lock(void) {
    system_mutex_acquire(f4_index.mutex);
}

procedure void F4_Index_Unlock(void) {
    system_mutex_release(f4_index.mutex);
}

function u64 _F4_Index_FileHash(Application_Links *app, Buffer_ID id) {
...
function F4_Index_File *_F4_Index_LookupFile(Application_Links *app, u64 hash, Buffer_ID buffer) {
...
procedure void F4_Index_EraseFile(Application_Links *app, Buffer_ID id) {
    ...
    if (file) {
        ...
        {
            // @Note(ema): The list is singly-linked, so we need another loop to get a prev pointer, then overwrite that.
...
procedure void _F4_Index_FreeNoteTree(F4_Index_Note *note) {
...
procedure void F4_Index_ClearFile(F4_Index_File *file) {
    if (file) {
        file->generation += 1;
...
procedure void F4_Index_InsertNote(F4_Index_ParseCtx *ctx, F4_Index_Note *note, Range_i64 name_range, F4_Index_NoteKind note_kind, F4_Index_NoteFlags note_flags) {
...
procedure void _F4_Index_Parse(Application_Links *app, F4_Index_File *file, String_Const_u8 string, Token_Array tokens, nicknema::Language *language) {
    ...
    if (language != 0)
        language->file_indexer(&ctx);
}

procedure void F4_Index_ParseFile(Application_Links *app, F4_Index_File *file, String_Const_u8 string, Token_Array tokens) {
    F4_Index_Lock();
    
    using namespace nicknema;
    
    Language *lang = language_from_buffer(app, file->buffer);
...
function b32 F4_Index_RequireToken(F4_Index_ParseCtx *ctx, String_Const_u8 string, F4_Index_TokenSkipFlags flags) {
    ...
    if (token) {
        String_Const_u8 token_string = string_substring(ctx->string, Ii64(token->pos, token->pos+token->size));
...
procedure void F4_Index_ParseComment(F4_Index_ParseCtx *ctx, Token *token) {
    ...
    for (u64 i = 0; i < string.size; i += 1) {
        if (string.str[i] == '@') {
            ...
            break;
        } else if (i+4 < string.size && string_match(str8_lit("TODO"), string_substring(string, Ii64(i, i + 4)))) {
            Range_i64 range = Ii64(token);
            range.min += i;
...
procedure void F4_Index_SkipSoftTokens(F4_Index_ParseCtx *ctx, b32 preproc) {
    for (; !ctx->done;) {
        Token *token = token_it_read(&ctx->it);
        if (preproc) {
...
procedure void F4_Index_SkipOpTokens(F4_Index_ParseCtx *ctx) {
    int paren_nest = 0;
    for (; !ctx->done;) {
        Token *token = token_it_read(&ctx->it);
...
procedure void F4_Index_Tick(Application_Links *app) {
    Scratch_Block scratch(app);
    
    // @Note(ema): I guess this loop does this: for each buffer that was modified, re-parse the whole file.
